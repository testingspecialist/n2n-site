...
CONTENT = '/home/dflorida/N2N RESTOS/n2n-web-v2/content/es'
...
/* Cards */
.card { background: var(--bg-white); border: 1px solid var(--border); border-radius: calc(var(--radius)*1.5); padding: var(--s8); box-shadow: var(--shadow-sm); transition: box-shadow .2s, transform .2s, border-color .2s; min-width: 0; }
...
.card__title { font-size: 1.12rem; font-weight: 700; color: var(--navy); line-height: 1.25; margin-bottom: var(--s3); overflow-wrap: anywhere; }
.card__body { font-size: .93rem; color: var(--text-muted); line-height: 1.6; margin-bottom: var(--s4); overflow-wrap: anywhere; }
...
.pillar__body h3 { font-size: 1.05rem; font-weight: 700; color: var(--navy); margin-bottom: var(--s2); overflow-wrap: anywhere; }
.pillar__body p { font-size: .93rem; color: var(--text-muted); line-height: 1.6; overflow-wrap: anywhere; }
...
.footer__bottom-links a { color: rgba(255,255,255,.65); font-size: .83rem; }
...
@media (max-width: 640px) {
  ...
  .nav__links { display: none; flex-direction: column; position: absolute; top: 64px; left: 0; right: 0; background: var(--navy); padding: var(--s4) var(--s6); gap: var(--s2); border-bottom: 2px solid rgba(255,102,0,.3); max-height: calc(100vh - 64px); overflow-y: auto; overscroll-behavior: contain; }
...
.btn--ghost:focus-visible {
  ...
  box-shadow: 0 0 0 4px rgba(0,0,85,.25);
}
...
def page(title, desc, body, current='', noindex=False, url='', schema=None):
    robots_tag = '\n<meta name="robots" content="noindex">' if noindex else ''
    _url  = url if url else 'https://n2n.com.ar/'
    _title = f'{title} | N2N' if title else 'N2N'
    _desc  = desc if desc else 'Arquitectura Comercial Digital para B2B industrial.'
    _og = f"""
...
<link rel="canonical"            href="{_url}">"""
    ...
    if schema:
        ...
        _schema = f'<script type="application/ld+json">{_json.dumps(schema, ensure_ascii=False)}</script>'
...
<title>{_title}</title>
<meta name="description" content="{_desc}">{_og}
{_schema}<link rel="icon" href="/img/favicon.ico" sizes="any">
...
def bc_schema(items):
    ...
    for i, (name, url) in enumerate(items, 1):
        ...
        if url:
            el['item'] = f'https://n2n.com.ar{url}'
        elements.append(el)
    return {'@context': 'https://schema.org', '@type': 'BreadcrumbList', 'itemListElement': elements}

def inner_page(label, breadcrumb, title, desc, body_html, current='', extra_actions='', url='', schema=None):
...
<section class="section bg-white">
  <div class="container">
    ...
    return page(title, desc, body, current, url=url, schema=schema)
...
write(f'{OUT}/index.html', page(
    ...
    home,
    url='https://n2n.com.ar/',
    schema={
        "@context": "https://schema.org",
        "@graph": [
            {
                "@type": "Organization",
                "@id": "https://n2n.com.ar/#organization",
                "name": "NODE2NODE / N2N",
                "url": "https://n2n.com.ar",
                "logo": "https://n2n.com.ar/img/n2n-logo.svg",
                "description": "Arquitectura Comercial Digital para B2B industrial.",
                "address": {"@type": "PostalAddress", "addressCountry": "AR"}
            },
            {
                "@type": "WebSite",
                "@id": "https://n2n.com.ar/#website",
                "url": "https://n2n.com.ar",
                "name": "N2N",
...
write(f'{OUT}/framework/como-funciona/index.html', inner_page(
    ...
    url='https://n2n.com.ar/framework/como-funciona/',
    schema=bc_schema([('Inicio','/'),('Framework','/framework/como-funciona/'),(m.get('title','Framework'),'')]),
))
...
write(f'{OUT}/framework/componentes/index.html', inner_page(
    ...
    url='https://n2n.com.ar/framework/componentes/',
    schema=bc_schema([('Inicio','/'),('Framework','/framework/como-funciona/'),('Componentes','')]),
))
...
for fname, slug, label in [
    ...
    write(f'{OUT}/industrias/{slug}/index.html', inner_page(
        ...
        url=f'https://n2n.com.ar/industrias/{slug}/',
        schema=bc_schema([('Inicio','/'),('Industrias','/industrias/'),(m.get('label',label),'')]),
    ))
...
for fname, slug, label in [
    ...
    write(f'{OUT}/servicios/{slug}/index.html', inner_page(
        ...
        url=f'https://n2n.com.ar/servicios/{slug}/',
        schema=bc_schema([('Inicio','/'),('Servicios','/servicios/'),(m.get('label',label),'')]),
    ))
...
project_files = sorted(_glob.glob(f'{CONTENT}/projects/*.md'))
proj_cards = ''
...
for fpath in project_files:
    ...
    estado_badge = ''
    if estado == 'activo':
        estado_badge = '<span style="display:inline-block;background:rgba(0,180,80,.12);color:#0a7a3a;border:1px solid rgba(0,180,80,.3);border-radius:100px;padding:.15em .8em;font-size:.72rem;font-weight:700;letter-spacing:.06em;text-transform:uppercase;margin-left:.5em">Activo</span>'
    elif estado == 'en desarrollo':
        estado_badge = '<span style="display:inline-block;background:rgba(255,102,0,.12);color:var(--orange);border:1px solid rgba(255,102,0,.3);border-radius:100px;padding:.15em .8em;font-size:.72rem;font-weight:700;letter-spacing:.06em;text-transform:uppercase;margin-left:.5em">En desarrollo</span>'

    url_link = f'<a href="{url}" target="_blank" rel="noopener noreferrer" style="font-size:.82rem;color:var(--text-muted)">{url}</a>' if url else ''

    b = f'''
<section class="hero" aria-labelledby="page-h1">
  <div class="container">
    <p class="breadcrumb"><a href="/">Inicio</a><span>\u203a</span><a href="/proyectos/">Proyectos</a><span>\u203a</span>{label}</p>
    <p class="hero__label">{tipo.capitalize()} &mdash; {cliente}</p>
    <h1 class="hero__h1" id="page-h1">{title} {estado_badge}</h1>
    <p class="hero__lead">{desc}</p>
    <div class="hero__actions">
      <a href="/contacto/" class="btn btn--primary">Hablemos \u2192</a>
      <a href="/proyectos/" class="btn btn--outline-white">Ver todos los proyectos</a>
    </div>
  </div>
</section>
<section class="section bg-white">
  <div class="container">
    <div style="margin-bottom:var(--s8);padding:var(--s5) var(--s6);background:var(--bg-section);border-radius:var(--radius);border:1px solid var(--border)">
      <p style="font-size:.75rem;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:var(--orange);margin-bottom:var(--s2)">Stack</p>
      <p style="font-size:.92rem;color:var(--text-muted)">{stack}</p>
      {f'<p style="margin-top:var(--s3)">{url_link}</p>' if url_link else ''}
    </div>
    <div class="prose">{md_to_html(bmd)}</div>
    <div style="margin-top:var(--s10);padding-top:var(--s8);border-top:1px solid var(--border);display:flex;gap:var(--s4);flex-wrap:wrap">
      <a href="/contacto/" class="btn btn--primary">Hablemos \u2192</a>
      <a href="/proyectos/" class="btn btn--ghost">\u2190 Ver todos los proyectos</a>
    </div>
  </div>
</section>'''
    write(f'{OUT}/proyectos/{slug}/index.html', page(title, desc, b, 'proyectos', url=f'https://n2n.com.ar/proyectos/{slug}/', schema=bc_schema([('Inicio','/'),('Proyectos','/proyectos/'),(title,'')])))
    proj_cards += f'''<a href="/proyectos/{slug}/" class="card card--link">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:var(--s2)">
        <div class="card__tag">{tipo}</div>{estado_badge}
      </div>
      <h2 class="card__title">{title}</h2>
      <p class="card__body">{desc[:140]}\u2026</p>
      <p style="font-size:.78rem;color:var(--text-muted);margin-top:var(--s2)">{stack[:80]}\u2026</p>
      <span class="card__link">Ver proyecto \u2192</span>
    </a>\n'''

proj_idx_body = f'''
<section class="hero" aria-labelledby="page-h1">
  <div class="container">
    <p class="hero__label">Proyectos</p>
    <h1 class="hero__h1" id="page-h1">Proyectos y Sistemas en Producci\u00f3n</h1>
    <p class="hero__lead">Sistemas operativos reales dise\u00f1ados y construidos por N2N para operaciones industriales y comerciales B2B latinoamericanas.</p>
    <div class="hero__actions">
      <a href="/contacto/" class="btn btn--primary">Hablemos de tu operaci\u00f3n \u2192</a>
    </div>
  </div>
</section>
<section class="section bg-white">
  <div class="container">
    <div class="grid grid--auto">{proj_cards}</div>
  </div>
</section>'''
write(f'{OUT}/proyectos/index.html', page('Proyectos \u2014 N2N', 'Sistemas y plataformas construidas por N2N para operaciones industriales B2B.', proj_idx_body, 'proyectos'))
...
<section class="section bg-white">
  <div class="container">
    ...
    write(f'{OUT}/conocimiento/{slug}/index.html', page(title, desc, b, 'conocimiento', url=f'https://n2n.com.ar/conocimiento/{slug}/', schema=bc_schema([('Inicio','/'),('Conocimiento','/conocimiento/'),(title,'')])))
...
<section class="section bg-white">
  <div class="container">
    ...
    write(f'{OUT}/comparar/{slug}/index.html', page(title, desc, b, url=f'https://n2n.com.ar/comparar/{slug}/', schema=bc_schema([('Inicio','/'),('Comparar','/comparar/'),(title,'')])))
        ...
        <form id="contact-form" novalidate aria-describedby="form-msg">
          <div class="form-grid">
            <div class="form-group">
              ...
              <input type="text" id="f-name" name="name" placeholder="Juan Garc\u00eda \u2014 Industrias Garc\u00eda SA" required minlength="3" maxlength="120" autocomplete="name">
            </div>
            <div class="form-group">
              <label for="f-email">Email corporativo *</label>
              <input type="email" id="f-email" name="email" placeholder="jgarcia@industriasgarcia.com" required maxlength="160" autocomplete="email" inputmode="email">
            </div>
            <div class="form-group">
              <label for="f-sector">Sector / operaci\u00f3n *</label>
              <select id="f-sector" name="sector" required autocomplete="organization-title">
            ...
            <div class="form-group">
              ...
              <textarea id="f-problem" name="problem" placeholder="Describ\u00edlo brevemente: qu\u00e9 ocurre hoy, qu\u00e9 quer\u00e9s resolver, en qu\u00e9 escala opera tu empresa." required minlength="20" maxlength="2000"></textarea>
            </div>
            <div class="form-row">
      ...
      <div style="display:flex;flex-direction:column;gap:var(--s6)">
        ...
        <div style="display:flex;flex-direction:column;gap:var(--s4)">
          ...
          <div>
            ...
            <a href="https://wa.me/5491138230614" target="_blank" rel="noopener noreferrer" style="color:var(--navy);font-weight:600">+54 9 11 3823-0614</a>
          </div>
...
_base  = 'https://n2n.com.ar'
_rutas = ['/', '/404.html', '/framework/como-funciona/', '/framework/componentes/', '/framework/', '/industrias/manufactura/', '/industrias/distribuidores/', '/industrias/operadores-logisticos/', '/industrias/', '/servicios/arquitectura-comercial/', '/servicios/plataformas-industriales/', '/servicios/consultoria-infraestructura/', '/servicios/', '/proyectos/stockfl/', '/proyectos/grillafl/', '/proyectos/tablero/', '/proyectos/romaneo/', '/proyectos/panel-admin/', '/proyectos/panel-finanzas/', '/proyectos/infraestructura-df/', '/proyectos/b2b-df/', '/proyectos/leads-horeca/', '/proyectos/presupuestos-rblimpieza/', '/proyectos/n2n-web/', '/proyectos/erpnext/', '/proyectos/', '/conocimiento/que-es-arquitectura-comercial-digital/', '/conocimiento/por-que-falla-ecommerce-b2b-industrial/', '/conocimiento/sistemas-calificacion-b2b/', '/conocimiento/precios-complejos-ventas-volumen/', '/conocimiento/integracion-erp/', '/conocimiento/portales-roles-permisos/', '/conocimiento/superficie-especificacion-comprador/', '/conocimiento/kpis-ruido-vs-trafico/', '/conocimiento/secuencia-implementacion/', '/conocimiento/marketing-despues-estructura/', '/conocimiento/guia-ceo-digital-industrial/', '/conocimiento/playbook-gerente-ventas/', '/conocimiento/', '/comparar/ecommerce-b2b-vs-arquitectura-comercial/', '/comparar/custom-vs-saas/', '/comparar/n2n-vs-agencia/', '/comparar/', '/contacto/', '/glosario/', '/legal/']
_urls  = []
for _r in _rutas:
    _prio = '1.0' if _r == '/' else '0.8'
    _urls.append(f'  <url>\n    <loc>{_base}{_r}</loc>\n    <lastmod>{_today}</lastmod>\n    <changefreq>monthly</changefreq>\n    <priority>{_prio}</priority>\n  </url>')
_sitemap = '<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n' + '\n'.join(_urls) + '\n</urlset>'
open(os.path.join(OUT, 'sitemap.xml'), 'w', encoding='utf-8').write(_sitemap)
